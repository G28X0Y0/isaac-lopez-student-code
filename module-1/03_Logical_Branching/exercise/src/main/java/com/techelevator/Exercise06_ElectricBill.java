...
public class Exercise06_ElectricBill {
    ...
    public double calculateElectricBill(double unitsUsed) {
        double totalCost = 0;
        double excessUnits = unitsUsed - EXCESS_UNITS_LIMIT;
        if (unitsUsed > EXCESS_UNITS_LIMIT) {
            totalCost = ((unitsUsed - excessUnits) * BASE_RATE) + (excessUnits * EXCESS_RATE);
        } else {
    ...
    public double calculateElectricBill(double unitsUsed, double unitsReturned) {
        ...
        //Alias discount
        double discountPercentage = DISCOUNT_FACTOR;
        //Assign total var
        double totalCost = 0;

        //Check if usage over limit
        if(netUsage > EXCESS_UNITS_LIMIT) {
            //Use base rate for 100 units and excess rate for remaining units
            totalCost = (EXCESS_UNITS_LIMIT * .20) + ((netUsage - EXCESS_UNITS_LIMIT) * EXCESS_RATE);
        }
        else {
            //Do base rate if under limit
            totalCost = netUsage * BASE_RATE;
        }
        //Check units returned vs units used
        if(unitsReturned > unitsUsed){
            totalCost = netUsage * .20;
        }
