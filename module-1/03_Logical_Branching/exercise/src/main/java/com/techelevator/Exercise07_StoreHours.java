...
public class Exercise07_StoreHours {
    ...
    public boolean isStoreOpen(int currentHour, char currentDay, boolean isSummer) {
        if (currentHour >= 8 && currentHour < 17 && (currentDay == 'M' || currentDay == 'W' || currentDay == 'F') && isSummer == false) {
            return true;
        }
        else if (isSummer){
            if(currentHour >= 8 && currentHour < 17 && (currentDay == 'M' || currentDay == 'F') || currentHour >= 8 && currentHour < 20 && currentDay =='W' || currentHour >= 9 && currentHour < 15 && currentDay == 'S'){
             return true;
            }
        }
        return false;
